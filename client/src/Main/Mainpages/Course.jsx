    ...
    const loadMoreCourses = () => {
        ...
        setTimeout(() => {
            setDisplayedCourses((prev) =>
                Math.min(prev + 8, coursesData.length)
            );
            setIsLoading(false);
        }, 500);
    };

    const coursesToShow = coursesData.slice(0, displayedCourses);
    const hasMore = displayedCourses < coursesData.length;
    ...
    return (
      ...
      `}</style>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* End Message */}
                {!hasMore && coursesData.length > 8 && (
                    <p className="text-center text-gray-600 font-medium">
                        You've viewed all {coursesData.length} courses
                    </p>
                )}
            </div>
        </section>
