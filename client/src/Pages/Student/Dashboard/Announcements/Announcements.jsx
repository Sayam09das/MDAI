...
const Announcements = () => {
    ...
    const [stats, setStats] = useState({ total: 0, forAll: 0, forStudents: 0 });
    ...
    const fetchAnnouncements = async () => {
        ...
        try {
            ...
            const res = await fetch(`${BACKEND_URL}/api/student/announcements?${params}`, getAuthHeaders());
            ...
            if (data.success) {
                ...
                if (filterType !== 'all') {
                    filtered = formattedAnnouncements.filter(ann => {
                        if (filterType === 'all-announcements') return true;
                ...
                setStats({
                    ...
                    forStudents: formattedAnnouncements.filter(a => a.type === 'students').length
                });
            } else {
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-slate-900">{stats.forStudents}</p>
                                <p className="text-sm text-slate-600">For Students Only</p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div className="flex-1">
                                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            <span className={`px-2 py-0.5 text-xs rounded-full ${announcement.type === 'all' ? 'bg-indigo-100 text-indigo-700' : announcement.type === 'students' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                                {announcement.type === 'all' ? 'All Users' : announcement.type === 'students' ? 'Students' : 'Teachers'}
                                            </span>
                                        </div>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setSelectedAnnouncement(null)}>
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg ${selectedAnnouncement.type === 'all' ? 'bg-indigo-100' : selectedAnnouncement.type === 'students' ? 'bg-green-100' : 'bg-amber-100'}`}>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                            <div className="text-sm text-slate-500">
                                ...
                                <span className={`px-2 py-0.5 text-xs rounded-full ${selectedAnnouncement.type === 'all' ? 'bg-indigo-100 text-indigo-700' : selectedAnnouncement.type === 'students' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                    {selectedAnnouncement.type === 'all' ? 'All Users' : selectedAnnouncement.type === 'students' ? 'Students' : 'Teachers'}
                                </span>
                            </div>
                            <button
