...
export const generateReceiptPdf = async (enrollment) => {
    ...
    /* HEADER */
    doc.fontSize(20).text("PAYMENT RECEIPT", { align: "center" }).moveDown(1);
    ...
    doc.font("Helvetica").text(`Name: ${enrollment.student.fullName}`);
    doc.moveDown();

    /* COURSE INFO */
    doc.font("Helvetica-Bold").text("Course Details");
    doc.font("Helvetica").text(`Course Name: ${enrollment.course?.title || "N/A"}`);
    doc.moveDown();

    /* PAYMENT INFO */
    doc.font("Helvetica-Bold").text("Payment Details");
    doc.font("Helvetica").text(`Amount Paid: ₹${enrollment.amount || "0"}`);
